...
class StickerDownloader:
    ...
    def download_file_io(self, link):
        res = self.session.get(link)
        return res.content
    ...
    def convert_file(_input, _output):
        command = '.\\libwebp\\bin\\dwebp.exe -quiet "{}" -o "{}"'.format(_input, _output)
...
#         downloader.convert_to_pngs(sset)
    downloader = StickerDownloader(TOKEN)
    pack = downloader.get_sticker_set("JFanimate")
    print(pack["files"][0].link)